...
func TestRouteAT_A2A(t *testing.T) {
	...
	r := newRegistry(cfg)
	now := time.Now()
	tx, _, err := r.CreateOrReplace(1, "AAL1", "", now)
	if err != nil {
		t.Fatal(err)
	}
	rx, _, err := r.CreateOrReplace(2, "AAL2", "", now)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, _, err := r.UpdateTransceivers(2, "AAL2", []Transceiver{{
		ID: 0, Frequency: 118700000, LatDeg: 40.01, LonDeg: -73.01, HeightMslM: 100,
	}}); err != nil {
		t.Fatal(err)
	}
	// bind both
	if _, _, ok := r.BindUDP(tx, fakeAddr{"127.0.0.1:1"}, now); !ok {
		t.Fatal("bind tx")
	}
	if _, _, ok := r.BindUDP(rx, fakeAddr{"127.0.0.1:2"}, now); !ok {
		t.Fatal("bind rx")
	}
